 ...
 * products.js
 * ------------------------------------------------------------------
 * Contém os dados de demonstração (usados como fallback) e a camada
 * de acesso a dados (getProducts) que busca o JSON gerado pelo
 * scraper via GitHub Actions. Veja getProducts() mais abaixo.
 * ------------------------------------------------------------------
 */
...
/**
 * Camada de acesso a dados. Busca o arquivo estático gerado
 * periodicamente pelo GitHub Actions (scraper/build_products_json.py
 * → data/products.json). Se o arquivo ainda não existir, estiver
 * vazio, ou o fetch falhar por qualquer motivo, cai de volta pros
 * dados de demonstração — assim a interface nunca fica quebrada.
 */
async function getProducts() {
  try {
    const response = await fetch("data/products.json", { cache: "no-store" });
    if (!response.ok) throw new Error("Resposta não OK ao buscar data/products.json");

    const payload = await response.json();
    if (Array.isArray(payload.products) && payload.products.length > 0) {
      window.__promoradarDataSource = { isDemo: false, generatedAt: payload.generatedAt };
      return payload.products;
    }
    throw new Error("data/products.json ainda está vazio");
  } catch (err) {
    console.warn("Usando dados de demonstração:", err.message);
    window.__promoradarDataSource = { isDemo: true, generatedAt: null };
    return demoProducts;
  }
}
